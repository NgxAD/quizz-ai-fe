...
export default function AIChat({ onQuestionsGenerated }: AIChatProps) {
  ...
  useEffect(() => {
    // Just show greeting message without creating quiz
    // Quiz will be created on first message
    setMessages([
      {
        id: '0',
        role: 'assistant',
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Initialize quiz on first message if not already done
      let currentConversationId = conversationId;
      let currentQuizId = quizInfo.quizId;

      if (!currentQuizId) {
        const initResponse = await aiApi.initializeAIChat();
        currentConversationId = initResponse.data.conversationId;
        currentQuizId = initResponse.data.quizId;
        setConversationId(currentConversationId);
        setQuizInfo({
          quizId: currentQuizId,
          quizTitle: initResponse.data.quizTitle,
        });
      }
